<script lang="ts">
	...
	let count = 0;
    let inputVal = ''
    const todos = writable<{id: Date | Number, text: string, completed: boolean}[]>([]);
    const photos = writable<{id: Number, thumbnailUrl: string, title: string}[]>([])
    ...
    const handleChange = () => {
        todos.update(items => [...items, { id: Date.now(), text: inputVal, completed: false }]);
        inputVal = ''
    }
